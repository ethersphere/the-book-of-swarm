...
\longnewglossaryentry{routability}{name=routability}{The ability for a chunk to be routed to destination.}
...
\longnewglossaryentry{chunk}{name=chunk, plural=chunks}{A chunk is fixed sized data blob, the basic unit stored in Swarm's DISC keyed by its address. There are content addressed chunks and single owner chunks.}
...
\longnewglossaryentry{content addressed chunk}{name=content addressed chunk, plural=content addressed chunks}{A chunk is Content addressing means that the chunk content determines the chunk address, usually the address being the fingerprint of the data using some hash function. The default content addressed chunk in Swarm is at most 4 kilobytes and uses Binary Merkle Tree hash with Keccak256 base hash as its address function.}
\longnewglossaryentry{single owner chunk}{name=single owner chunk, plural=single owner chunks}{A special type of chunk in Swarm, the integrity of which is given by the association of its payload to an identifier attested by the signature of its owner while the identifier and the owner's account determines the chunk address.}
...
\longnewglossaryentry{routed delivery}{name=routed delivery}{Hypothetical way of chunk delivery using kademlia routing but independently of the request.}
...
\longnewglossaryentry{access key decryption key}{name=access key decryption key}{The key the publisher grants to party in multi party selective access scenario, to decrypt the global access key.}
...
\longnewglossaryentry{neighbourhood notification}{name=neighbourhood notification}{Notification about feed update which works without the issuer of the notification knowing the identity of prospective posters.}
...
\longnewglossaryentry{distributed immutable store for chunks}{name=distributed immutable store for chunks}{Swarm's version of distributed hash table for storing files, where it does not keep a list of where files are to be found, instead it actually stores pieces of the file directly on the node.}
\longnewglossaryentry{distributed hash table}{name=distributed hash table,plural=distributed hash tables}{A distributed system that provides a lookup service where any participating node can efficiently retrieve the value associated with a given key. }
\longnewglossaryentry{binary Merkle tree}{name={binary Merkle tree}}{A binary tree in which every leaf node is labelled with the cryptographic hash of a data block, and every non-leaf node is labelled with hash of labels of its child nodes.}
...
\longnewglossaryentry{bzz account}{name=bzz account}{The account in Swarm, also see \gloss{Swarm base account}.}
\longnewglossaryentry{enode URL scheme}{name=enode URL scheme}{An URL scheme under which an Ethereum node can be described.}
...
\longnewglossaryentry{chunk value}{name=chunk value}{The chunk value is determined  by the price of the postage batch it is stamped with. It serves to order chunks when a node prioritises for garbage collection.}
...
\longnewglossaryentry{range queries}{name=range queries}{Range queries will trigger the retrieval of all and only those chunks of the file that cover the desired range.}
\longnewglossaryentry{BZZ network ID}{name=bzz network ID}{The ID of the Swarm network.}
\longnewglossaryentry{Kademlia}{name=Kademlia}{A network connectivity or routing scheme based on bitprefix length used in distributed hash tables.}
...
\longnewglossaryentry{uploader}{name=uploader}{The client uploading content to the Swarm network.}
...
\longnewglossaryentry{aligned incentives}{name=aligned incentives}{Rewarding / penalization in such a way, that the actors are inclined towards desired behavior.}
\longnewglossaryentry{forwarding lag}{name=forwarding lag}{The time in which healthy nodes could forward messages.}
\longnewglossaryentry{sender anonymity}{name=sender anonymity}{As requests are relayed from peer-to-peer, further down on the request cascade can never know who the originator of the request is.}
\longnewglossaryentry{decentralised network}{name=decentralised network}{A network designed with no central node upon which the other nodes would need to depend on.}
...
\longnewglossaryentry{redundancy}{name=redundancy}{In the context of distributed chunk store, redundancy is a surplus replicas or so called parities that contribute to resilience of chunk storage in the face of churn and garbage collection.}
...
\longnewglossaryentry{postage batch}{name=postage batch, plural={postage batches}}{A postage batch is an id associated with a verifiable payment on chain which can be attached to (multiple) chunks as a postage stamp.}
...
\longnewglossaryentry{simple ordered sequence}{name=simple ordered sequence}{An indexing scheme and a feed type using this scheme, where subsequent updates indexes are incrementing integers.}
...
\longnewglossaryentry{root access manifest}{name=root access manifest}{A special unencrypted manifest used in entry point for access control.}
\longnewglossaryentry{witness batch}{name=witness batch}{Serves as a spot check for the applicant's claim that they store all chunks they are responsible for. The witness batch is random choice of the valid postage batches; the applicant must have all chunks in their neighbourhood.}
...
\longnewglossaryentry{partitions}{name=partitions, parent={feeds}}{Special kind of feeds, updates of which are meant to accumulated or added to earlier ones, e.g., parts of a video stream.}
...
\longnewglossaryentry{pub-sub systems}{name={pub\slash sub systems}}{Publish/subscribe system is a form of asynchronous communication where any message published is immediately received by subscribers.}
\longnewglossaryentry{Ethereum Name Service}{name=Ethereum Name Service}{The system that, analogously to the DNS of the old web, translates human-readable names into system-specific identifiers, i.e., reference in the case of Swarm.}
...
\longnewglossaryentry{outbox}{name=outbox}{TBD}
...
\longnewglossaryentry{feed aggregation}{name=feed aggregation}{The process of turning a set of sporadic feeds into a periodic one.}
\longnewglossaryentry{outbox feed}{name=outbox feed}{A feed representing the outgoing messages of a persona.}
\longnewglossaryentry{extended triple Diffie--Hellmann key exchange}{name=extended triple Diffie--Hellmann key exchange}{The customary method of establishing the initial parameters of the double ratchet key chains.}
\longnewglossaryentry{future secrecy}{name=future secrecy}{A feature of specific key agreement protocols that gives assurances that session keys will not be compromised even if the private key of the server is compromised.}
...
\longnewglossaryentry{real-time integrity check}{name=real-time integrity check}{For any deterministically indexed feed. Integrity translates to non-forking or unique chain commitment.}
...
\longnewglossaryentry{pre-key bundle}{name=pre-key bundle}{Consists of all information the initiator needs about responder to initiate a handshake.}
...
\longnewglossaryentry{anonymous retrieval}{name=anonymous retrieval}{Not disclosing the identity of requestor node while retrieving a chunk.}
...
\longnewglossaryentry{upstream peer}{name=upstream peer}{The peer that is preceeding some other peer in the chain of forwarding.}
...
\longnewglossaryentry{downstream peer}{name=downstream peer, parent={peer}}{The peer that is succeeding some other peer in the chain of forwarding.}
\longnewglossaryentry{disconnect threshold}{name=disconnect threshold}{The threshold for debt between peers that determines the value of debt at which a peer in debt will be disconnected.}
...
\longnewglossaryentry{prefix collision}{name=prefix collision}{In the context of postage stamps, when owner of a batch attaches it to two chunks with a shared prefix longer than the depth of the batch.}
\longnewglossaryentry{collision slot}{name=collision slot, plural= collision slots}{The collection of maximum length prefixes that any two chunk stamped with a postage batch are allowed to share. A stamped chunk occupies a collision slot. }
...
\longnewglossaryentry{Cauchy-Reed-Solomon erasure code}{name=Cauchy-Reed-Solomon erasure code}{A systemic erasure code which when applied to a data (of $n$ chunks)  produces $k$ extra 'parity' chunks  in such a way that any $n$ chunks out of the total $n + k$ are enough to reconstruct the original blob.}
\longnewglossaryentry{missing chunk notification protocol}{name=missing chunk notification protocol}{A protocol upon which downloader not finding a chunk, can fall back to a recovery process and request the chunk from a pinner.}
\longnewglossaryentry{recovery}{name=recovery}{A process of requesting a missing chunk from specific recovery hosts.}
\longnewglossaryentry{recovery request}{name=recovery request}{A request to a recovery host to initiate a reupload of a missing chunk we know it has pinned.}
...
\longnewglossaryentry{recovery response envelope}{name=recovery response envelope}{An addressed envelope to provide a way for recovery hosts to  respond directly to the originator of the recovery request promptly and without cost or computational burden.}
...
\longnewglossaryentry{Web 2.0}{name=Web 2.0, parent={World Wide Web}}{Describes websites that emphasize user-generated content, ease of use, participatory culture and interoperability for end users.}
\longnewglossaryentry{peer-to-peer}{name=peer-to-peer}{A network where tasks or workloads are partitioned between peers, which are equally privileged participants.}
...
\longnewglossaryentry{incentive strategy}{name=incentive strategy}{A strategy of rewarding and penalizing behavior to reach desired goals.}
...
\longnewglossaryentry{collective information}{name=collective information}{Data produced by collective effort, such as data on public forums, reviews, votes, polls.}
\longnewglossaryentry{data silo}{name=data silo, plural={data silos}}{A collection of information in an organization that is isolated from and not accessible by other parts of the organization. In more general meaning also the data large companies are keeping to themselves.}
...
\longnewglossaryentry{overlay network}{name=overlay network}{Connectivity pattern of Swarm oecondary conceptual network .}
...
\longnewglossaryentry{devp2p}{name=devp2p}{A set of network protocols which form the Ethereum peer-to-peer network. Also implemented as set of programming libraries with the same name.}
...
\longnewglossaryentry{nearest neighbours}{name=nearest neighbours}{Generally, peers that are closest to the node. In particular, }
...
\longnewglossaryentry{stream provider}{name=stream provider}{TBD Viktor}
\longnewglossaryentry{bin ID}{name=bin ID}{A sequential counter  per PO bin acting as index of the chunks stored locally on a node.}
...
\longnewglossaryentry{head start}{name=head start}{A configurable time interval that defines waiting time before an exploration of an area for updates of epoch-based feeds. }
...
\longnewglossaryentry{fair data economy}{name=fair data economy}{Economy of processing data with fair compensation of all parties involved in its creation or enrichment.}
...
\longnewglossaryentry{denial of service (DoS)}{name=denial of service (DoS)}{Denying of access to services by flooding those services with illegitemate requests.}
...
\longnewglossaryentry{shallow bin}{name={shallow bin}, plural={shallow bins} }{TBD Dan}
\longnewglossaryentry{deep bin}{name={deep bin}, plural={deep bins}}{TBD Dan}
\longnewglossaryentry{on-chain payment}{name={on-chain payment}, plural= {on-chain payments}}{Payment via the blockchain network. Each transaction must be processed by each and every node in the network leading to relatively high transaction costs.}
\longnewglossaryentry{second-layer payment}{name={second-layer payment}, plural={second-layer payments} }{TBD}
...
\longnewglossaryentry{blockhash}{name=blockhash}{The top hash of a block of data, e.g. a Merkle tree.}


\longnewglossaryentry{recover security}{name=recover security}{TBD}
\longnewglossaryentry{recovery targets}{name=recovery targets}{TBD Dan}
...
\longnewglossaryentry{key derivation  function}{name=key derivation  function}{TBD}
