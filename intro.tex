...
On (or after) 6:15 Saturday the 3rd of January 2009, the world changed forever. A mysterious Cypherpunk created the first block of a chain that would come to encircle the entire world, and the genie was out of the bottle. This first step would put in motion a set of reactions that would result in an unprecedentedly humongous amount of money flowing from the traditional reservoirs of fiat and physical goods into a totally new vehicle to store and transmit value: cryptocurrency. 'Satoshi Nakamato' had managed to do something no-one else had been able to. He had, de facto, though at small scale, disintermediated the banks by creating the decentralised, trustless transfer of value. Since that moment, we are effectively back to a world where multiple kinds of money can exist. Everyone can now be their own central bank, own their own central-bank money and their own money-transmission system. Money they can print but that no-one else can multiply or inflate out of their pocket. It is still not well understood how much this will change our economies.
...
Despite this huge amount of value, the asymmetric distribution of the wealth generated by the existing data economy has been put forward as a major humanitarian issue \cite{TheWinner2020Feb}. As efficiency and productivity continue to rise, as a result of better data, the resulting profits will need to be distributed. Today, the spoils are distributed unequally: the larger the companies' data set, the more they can learn from the data, attract more users and hence even more data. Currently, this is most apparent with the dominating large tech companies such as \gloss{FAANG}, but it is predicted that this will also be increasingly important in non-tech sectors, or even at the level of nation-states. Hence, companies are racing to become dominant in a particular sector, and countries hosting these platforms will gain an advantage. As Africa and Latin America host so few of these, they risk becoming exporters of raw data and then paying other countries to import the intelligence provided, as has been warned by the United Nations Conference on Trade and Development \cite{TheWinner2020Feb}. Another problem is that if a large company monopolises a particular data market, it could also become the sole purchaser of data, thereby maintaining a complete control of setting prices and affording the possibility that the "wages" for providing data could be manipulated to keep them artificially low. In many ways, we are already seeing evidence of this.
...
Artificial Intelligence (AI) is promising to bring about major changes to our society. On the one hand, it is envisioned to allow for a myriad of business opportunities, while on the other hand it is expected to displace many professions and jobs, rather than merely augmenting them \cite{Lee2018Sep}.
...
Using Swarm will allow us to leverage a fuller set of data and to create better services, while still having the option to contribute it to the global good with self-verifiable anonymisation. The best of all worlds.
...
\glossupper{collective information} began to accumulate first with the emergence of the Internet, yet the concept has just recently become recognized and discussed under a variety of headings such as  \emph{open source}, \emph{fair data} or \emph{information commons}.
...
\end{displayquote}
The internet allows collectives to be formed on a previously unthinkable scale, despite differences in geographic location and demographic aspects such as political convictions, social status, wealth, or even general freedom. Data produced by these collectives, through joint interaction on public forums, reviews, votes, repositories, articles and polls is a form of collective information---as is the metadata that emerges from the traces of these interactions. Since most of these interactions, today, are facilitated by for-profit entities running centralized servers, the collective information ends up being stored in data silos owned by a commercial entity, the majority of it concentrated in the hands of a few big technology companies. And while the actual work results are often in the open, as the offering of these providers, the metadata, which can often be the more valuable, powerful and dangerous representation of the interaction of contributors, is usually held and monetized secretly.
...
Inclusivity means that we aspire to include the underprivileged in the data economy. It also means to lower the barrier of entry to defining complex data flows and to building decentralised applications. Swarm is a network with open participation: for providing services and permissionless access to publishing, sharing, and investing your data.

Users are free to express their intention as 'action' and have full authority to decide if they want to remain anonymous or share their interactions and preferences. The integrity of online personae is required.

Economic incentives serve to make sure participants' behaviour aligns with the desired emergent behaviour of the network (see \ref{sec:incentivisation}).
...
For continuity, the solution must have several generic requirements expressed as \emph{systemic properties}. The solution must be:

\begin{itemize}
\item \emph{stable}---The specifications and software implementations must be stable and resilient to changes in participation or politics (political pressure, censorship).
\item \emph{scalable}---The solution must be able to accommodate many orders of magnitude more users and data than it starts out with, without leading to prohibitive reductions in performance or reliability during mass adoption.
\item \emph{secure}---The solution must be resistant to deliberate attacks and immune to social pressure and politics, as well as fault-tolerant in its technological dependencies (e.g. blockchain, programming languages).
\item \emph{self-sustaining}---The solution must run by itself as an autonomous system. It must not depend on human or organisational coordination of collective action, or on any legal entity's business, or on exclusive know-how, or on special hardware, or on any specific network infrastructure.
\end{itemize}
